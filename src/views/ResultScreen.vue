...
export default {
  ...
  async created () {
    try {
      ...
      const estresse = scores.sqr20 || scores.trabalho || scores.esgotamento

      if (estresse) {
        this.opcoesFinalizacao = ['dicas de saúde', 'unidades de saúde', 'encerrar']
        this.exibicao = { titulo: 'Você parece um pouco estressado(a)', descricao: 'Vamos te dar umas dicas de saúde e bem estar ok? <br> Não se esqueça de consultar as unidades de saúde, caso precise.', color: 'error', img: 'estresse', atribuicao: 'freepik' }
        this.encerrar = this.opcoesFinalizacao
      } else {
        this.opcoesFinalizacao = ['dicas de saúde', 'encerrar']
        this.exibicao = { titulo: 'Você parece bem!', descricao: 'Para que continue assim, não se esqueça de praticar hábitos saudáveis.', color: 'info', img: 'saude', atribuicao: 'esdesignisms' }
      }
    } catch (e) {
